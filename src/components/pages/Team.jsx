...
const GBCard = ({ member, index }) => {
    ...
    useEffect(() => {
        ...
        gsap.fromTo(ref.current,
            { y: 60, opacity: 0 },
            { y: 0, opacity: 1, duration: 0.7, ease: 'power3.out', delay: index * 0.08,
    ...
    return (
        <div ref={ref}
            className="group flex flex-col bg-white rounded-2xl border-2 border-neutral-100 overflow-hidden
                       transition-all duration-500 ease-out
                       hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(34,211,238,0.15)] hover:border-cyan-400/30">
            {/* Image */}
            <div className="relative overflow-hidden" style={{ paddingBottom: '125%' }}>
                <img
                    ...
                    className="absolute inset-0 w-full h-full object-cover object-top
                               grayscale group-hover:grayscale-0 transition-all duration-700 group-hover:scale-110"
                />
                ...
                <div className="absolute top-0 left-0 h-0.5 w-0 bg-cyan-400 group-hover:w-full transition-all duration-700" />
            </div>

            {/* Info */}
            <div className="px-4 pt-3 pb-2 border-t-2 border-neutral-100 flex flex-col gap-0.5">
                <p className="text-black font-bold text-sm md:text-base leading-tight tracking-tight truncate">{member.name}</p>
                <p className="text-cyan-400 text-[11px] md:text-xs font-bold tracking-widest uppercase truncate">{member.position}</p>
            </div>

            {/* Social row */}
            <div className="flex items-center justify-between px-4 py-3 mt-auto">
                <a href={member.linkedin} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1.5 text-neutral-400 hover:text-[#0A66C2] transition-colors duration-300 text-[11px] font-semibold tracking-wide">
                    <Linkedin size={14} /><span>LinkedIn</span>
                </a>
                {member.email ? (
                    <a href={`mailto:${member.email}`}
                        className="flex items-center gap-1.5 text-neutral-400 hover:text-cyan-400 transition-colors duration-300 text-[11px] font-semibold tracking-wide">
                        <Mail size={14} /><span>Email</span>
                    </a>
                ) : member.github ? (
                    <a href={member.github} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1.5 text-neutral-400 hover:text-black transition-colors duration-300 text-[11px] font-semibold tracking-wide">
...
const MemberCard = ({ member, index }) => {
    ...
    useEffect(() => {
        ...
        gsap.fromTo(ref.current,
            { y: 60, opacity: 0 },
            { y: 0, opacity: 1, duration: 0.7, ease: 'power3.out', delay: (index % 6) * 0.06,
    ...
    return (
        <div ref={ref}
            ...
            <div className="relative overflow-hidden" style={{ paddingBottom: '120%' }}>
                {member.image ? (
                    <img src={member.image} alt={member.name}
                        className="absolute inset-0 w-full h-full object-cover object-top
                                   grayscale group-hover:grayscale-0 transition-all duration-700 group-hover:scale-105" />
                ) : (
                    <div className="absolute inset-0">
                        <PhotoPlaceholder color={member.color} />
                    </div>
                )}
                <div className="absolute bottom-0 left-0 right-0 h-14 bg-gradient-to-t from-white to-transparent pointer-events-none" />
                <div className="absolute top-0 left-0 h-0.5 w-0 bg-cyan-400 group-hover:w-full transition-all duration-700" />
                {/* Department badge */}
                <div className="absolute top-2.5 right-2.5 bg-white/90 backdrop-blur-sm border border-neutral-200
                                rounded-full px-2.5 py-0.5 text-[9px] font-black text-cyan-400 tracking-widest uppercase
            ...
            {/* Info */}
            <div className="px-3.5 pt-3 pb-1.5 border-t-2 border-neutral-100 flex flex-col gap-0.5">
                <p className="text-black font-bold text-sm leading-tight tracking-tight truncate">{member.name}</p>
                <p className="text-cyan-400 text-[11px] font-bold tracking-widest uppercase truncate">{member.position}</p>

                {/* Portfolio link */}
                {member.portfolio ? (
                    <a href={member.portfolio} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1 text-neutral-400 hover:text-black transition-colors duration-300 text-[11px] font-medium mt-0.5 w-fit">
                        <Globe size={11} /><span>Portfolio</span><ExternalLink size={9} />
                    </a>
                ) : (
                    <span className="flex items-center gap-1 text-neutral-300 text-[11px] font-medium mt-0.5">
            ...
            <div className="flex items-center justify-between px-3.5 pb-3.5 mt-auto">
                ...
                {member.github ? (
                    ...
                    <span className="flex items-center gap-1.5 text-neutral-300 text-[11px] font-semibold tracking-wide cursor-default">
                        <Github size={13} /><span>GitHub</span>
                    </span>
                )}
            </div>
